...
> **Status:** delivered. Coverage gate (ratchet thresholds in `vitest.config.ts`, run in CI via `test:coverage`), KV-persistence + auth + snapshot integration tests, and an e2e + axe a11y job in CI (built app on a dedicated port). The a11y gate enforces structure/semantics; `color-contrast` is intentionally disabled in the gate and tracked as design debt below.
>
> **Known a11y debt (color-contrast):** the editorial design uses the theme accent at low opacity for secondary labels (fixtures dates/counts/filter tabs, ledger labels, etc.), which fails WCAG AA on the tinted grounds, and differently per theme. The decorative footer is exempt (WCAG 1.4.3, `data-decorative` + aria-hidden). The interactive leaderboard view-toggle was fixed by bumping `--color-fg-muted`/`--color-fg-subtle`. The remaining accent-text contrast needs a coherent cross-theme pass before re-enabling the rule in `e2e/a11y.spec.ts`.
...
| Cross-theme accent-text contrast pass, then re-enable `color-contrast` in the a11y gate; optionally add the leaderboard live-region announcement (draft-ceremony reduced-motion and the interactive-control contrast are already done) | fg-subtle-contrast-fail, live-scores-no-aria-live    | M      | Secondary/accent label text passes WCAG AA on all four themes; `color-contrast` is re-enabled in `e2e/a11y.spec.ts` and green; live leaderboard changes are announced. |
