...
| Raise `--color-fg-subtle` contrast and complete or downgrade the ARIA tabs pattern; add the live-region announcement and the draft-ceremony reduced-motion branch | fg-subtle-contrast-fail, incomplete-aria-tabs-pattern, live-scores-no-aria-live, draft-ceremony-no-reduced-motion | M      | Secondary/interactive text passes WCAG AA on all four themes; live leaderboard changes are announced; the reel-spin respects `prefers-reduced-motion`. |
