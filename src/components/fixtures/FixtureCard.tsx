  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="font-heading text-[10px] font-semibold md:text-[11px]" style={{ color: `${theme.accent}55` }}>
          {f.date} · {f.time}
        </span>
      </div>
      ...
      <div className="flex items-center justify-between px-4 py-4 md:px-5 md:py-5">
        <div className="flex-1">
          ...
          <div className="mt-0.5 text-[9px] font-medium md:text-[10px]" style={{ color: `${theme.accent}44` }}>{getOwner(f.t1)}</div>
        </div>

        <div className="shrink-0 px-2 text-center md:px-4">
          {hasScore ? (
            <div className="font-display flex items-center gap-2 md:gap-3" style={{ fontSize: 'clamp(28px, 6vw, 40px)' }}>
        ...
        <div className="flex-1 text-right">
          <div className="flex justify-end"><Flag team={f.t2} size={32} /></div>
          <div className="font-display mt-1.5 text-[12px] md:text-sm">{f.t2}</div>
          <div className="mt-0.5 text-[9px] font-medium md:text-[10px]" style={{ color: `${theme.accent}44` }}>{getOwner(f.t2)}</div>
        </div>
      </div>
