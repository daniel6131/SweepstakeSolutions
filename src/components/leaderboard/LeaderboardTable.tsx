...
export function LeaderboardTable({ entries, theme }: Props) {
  return (
    ...
    >
      ...
      >
        <span>#</span><span>PLAYER</span><span className="text-right">PTS</span>
      </div>
      ...
      {entries.map((p, i) => (
        <div key={p.name}>
          ...
          >
            <span className="font-display text-xl" style={{ color: i < 3 ? theme.accent : 'rgba(255,255,255,0.15)' }}>{i + 1}</span>
            <div>
              ...
              <div className="flex flex-wrap gap-1">
                {p.teams.map((t) => (
                  <div
                    ...
                    style={{ background: `${theme.accent}06`, border: `1px solid ${theme.accent}08`, color: 'rgba(255,255,255,0.4)' }}
                  >
            ...
            <span className="font-display text-right text-[32px]" style={{ color: p.pts > 0 ? theme.accent : 'rgba(255,255,255,0.08)' }}>{p.pts}</span>
          </div>
          ...
          >
            <span className="font-display text-base" style={{ color: i < 3 ? theme.accent : 'rgba(255,255,255,0.15)' }}>{i + 1}</span>
            <div className="min-w-0">
              <div className="text-[13px] font-bold">{p.name}</div>
              <div className="mt-0.5 flex gap-1 overflow-hidden">
                {p.teams.map((t) => <Flag key={t} team={t} size={12} />)}
              </div>
            ...
            <span className="font-display text-right text-[28px]" style={{ color: p.pts > 0 ? theme.accent : 'rgba(255,255,255,0.08)' }}>{p.pts}</span>
          </div>
        </div>
      ))}
    </div>
