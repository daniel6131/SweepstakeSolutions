...
type Props = { entry: LeaderboardEntry; rank: number; teamRecords: Map<string, TeamRecord>; theme: ThemeColors };
...
export function ParticipantCard({ entry, rank, teamRecords, theme }: Props) {
  return (
    ...
    >
      ...
      >
        <div>
          <div className="font-display leading-none" style={{ fontSize: 'clamp(22px, 4vw, 30px)', color: theme.accent }}>{entry.name}</div>
          <div className="font-heading mt-1.5 text-[11px] font-medium" style={{ color: 'rgba(255,255,255,0.25)' }}>
            {entry.pts} pts · {entry.w + entry.d + entry.l} games
          </div>
        </div>
        <div className="font-display leading-none" style={{ fontSize: 'clamp(36px, 6vw, 52px)', color: `${theme.accent}06` }}>{rank}</div>
      </div>
      ...
      <div className="p-2.5 md:p-3">
        {entry.teams.map((t) => {
          ...
          return (
            <div key={t} className="row-hover mb-0.5 flex items-center justify-between rounded-lg px-3 py-2.5 md:rounded-xl md:px-4 md:py-3">
              <div className="flex items-center gap-2.5 md:gap-3">
                <Flag team={t} size={26} />
                <div>
                  <div className="text-[13px] font-semibold">{t}</div>
                  <div className="text-[9px]" style={{ color: 'rgba(255,255,255,0.25)' }}>{r.w}W · {r.d}D · {r.l}L</div>
                </div>
              </div>
              <div className="flex items-center gap-2 md:gap-3">
                ...
                <span className="font-display w-8 text-right text-xl md:w-10 md:text-2xl" style={{ color: r.pts > 0 ? theme.accent : 'rgba(255,255,255,0.08)' }}>
                  {r.pts}
                </span>
              </div>
            </div>
